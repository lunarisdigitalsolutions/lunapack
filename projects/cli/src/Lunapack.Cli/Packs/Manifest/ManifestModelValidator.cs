...
internal static partial class ManifestModelValidator
{
    ...
    )
    {
        ...
        if (resolvedPack.GitSource is not { } provenance)
        {
            ValidateLocalResolvedPackSource(resolvedPack, issues);
        }
        else
        {
            ValidateGitResolvedPackSource(resolvedPack, provenance, issues);
        }
    ...
    )
    {
        ...
        if (
            managedFile.SourceAlias is not { } sourceAlias
            || !IsSourceAlias(sourceAlias)
    ...
    {
        if (string.IsNullOrEmpty(value) || IsAbsolutePath(value))
        {
            return false;
        }
