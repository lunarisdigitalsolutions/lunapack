...
public sealed class ProjectStateStoreTests
{
    ...
    [Test]
    public async Task Save_WhenLockSourceNameDoesNotMatch_ReturnsFailure()
    {
        var fileSystem = CreateFileSystem();
        const string projectDirectory = @"C:\project";
        fileSystem.AddDirectory(projectDirectory);
        ...
        var saved = await stateStore.SaveAsync(
            projectDirectory,
            CreateStateWithSourceIdentity("other", "source")
        );
    ...
    [Test]
    public async Task Save_WhenLockSourceIdentityDoesNotMatch_ReturnsFailure()
    {
        var fileSystem = CreateFileSystem();
        const string projectDirectory = @"C:\project";
        fileSystem.AddDirectory(projectDirectory);
        ...
        var saved = await stateStore.SaveAsync(
            projectDirectory,
            CreateStateWithSourceIdentity("local", "other-source")
        );
