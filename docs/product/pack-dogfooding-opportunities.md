...
This proposal identifies repository assets that could become reusable packs and
be consumed by LunaPack itself. It records opportunities only; it does not
define committed pack contracts or claim that the proposed packs exist.

## Current position

The repository already dogfoods 15 local packs from `projects/packs`. Its
`lunapack.yml` installs 12 roots covering .NET configuration, coding guidance,
an MIT license, ignore rules, and GitHub pull-request automation.

Two current roots are also dependencies of `dotnet-project`:
`dotnet-build-props` and `dotnet-central-package-management`. Luna resolves each
...
- `dotnet-project` and `github-actions-commitlint` use exact-version
  composition.
- `license-mit` uses a required parameter and Scriban template.
- Gitignore packs use section merges, while SDK and tool manifests use JSON
  merges.
- `commitlint` and `dotnet-csharpier-tool` use script hooks.
- `github-actions-pr-gate` uses a non-executable instruction hook.
...
| Pack | Assessment |
| --- | --- |
| `gitignore-general` | Good language-neutral section merge. |
| `dotnet-gitignore` | Good .NET-specific companion to general ignores. |
| `dotnet-csharpier-tool` | Good JSON merge and tool-restore hook example. |
| `license-mit` | Good parameterized template with explicit overwrite behavior. |
| `madr-adr-template` | Portable template, but its default target should align with any future ADR guidance pack. |
...
| `github-actions-pr-gate` | Reusable when consumers need to aggregate external checks. Its required-check setup is correctly an instruction, not a script. |
...
| `dotnet-build-props` | Separate baseline build policy from analyzer and TUnit package references if consumers need independent choices. It currently fixes `net10.0`, latest C#, CSharpier, Meziantou.Analyzer, TUnit, and filesystem test helpers as one policy. |
...
| `commitlint` | Remove LunaPack wording and make allowed scopes configurable. Current defaults permit only `frontend` and `backend`. |
| `github-actions-commitlint` | Describe it as PR-title validation. Current workflow does not validate linked Azure Boards work items despite its pack description. Parameterize the target branch and Node version only where consumers genuinely vary them. |
...
| --- | --- | --- |
| `editorconfig-general` | The root `.editorconfig` contains reusable defaults for Markdown, YAML, shell, batch, Makefile, and TypeScript. | Move C# rules to a separate contributor. Compose contributors using deterministic section or whole-file ownership after testing update behavior. |
...
| --- | --- | --- |
| `dotnet-repository` | `gitignore-general`, `dotnet-gitignore`, `dotnet-sdk-10`, `dotnet-editorconfig`, `dotnet-csharpier-tool`, `dotnet-project` | Reproduce the repository's core .NET baseline without changing the narrower `dotnet-project` contract. |
...
| `github-pr-quality` | refined `github-actions-commitlint`, `github-actions-pr-gate` | Install title validation and external-check gating as one GitHub profile. Keep each workflow independently installable. |
| `repository-documentation-quality` | `markdownlint-config`, `prettier-standard-config`, future `github-copilot-documentation-instructions`, `madr-adr-template` | Align formatting, linting, ADR authoring, and agent guidance. Keep Node dependency installation opt-in. |
...
1. Correct current pack descriptions and boundaries, especially Commitlint,
   PR-title validation, EditorConfig, and bundled .NET test policy.
2. Create and dogfood `markdownlint-config` and
   `prettier-standard-config`; both have small surfaces and immediate local
   evidence.
3. Generalize community-health and scoped documentation-instruction content
   using parameters, conditions, and path-aware templates.
4. Package the .NET build composite action with its Microsoft Testing Platform
   prerequisites stated explicitly.
5. Add `dotnet-repository`, `github-pr-quality`, and
   `repository-documentation-quality` only after every child passes standalone
   install, update, dry-run, and uninstall checks.
6. Change this repository to consume each new local pack, remove the source
   files it replaces from manual ownership, and verify a clean reinstall from
   `lunapack.yml` plus the lock file.
7. Validate portable candidates in at least one unrelated repository before
