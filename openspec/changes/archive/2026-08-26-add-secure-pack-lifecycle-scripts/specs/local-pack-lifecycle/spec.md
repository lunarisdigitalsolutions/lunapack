...
For each script requiring consent, LunaPack SHALL show a structured confirmation containing the pack ID, hook type, optional description, and exact executable and arguments that will run. LunaPack SHALL fail closed without executing scripts or mutating the operation when consent is declined, input is unavailable, or authorization cannot be established. `--dry-run` SHALL execute no scripts and SHALL report each planned hook with its selected consent mode.
